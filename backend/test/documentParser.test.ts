...
describe('payslip interpretation', () => {
  ...
  it('supplements one labelled total without inventing the other totals', async () => {
    const previousGeminiKey = process.env.GEMINI_API_KEY;
    const previousFetch = globalThis.fetch;
    process.env.GEMINI_API_KEY = 'test-gemini-key';
    globalThis.fetch = async () => emptyGeminiPayslipResponse();
    ...
    try {
      ...
      const fields = result.summary.extractedFields as PayslipInterpretation;
      assert.equal(fields.grossEarnings, null);
      assert.equal(fields.totalDeductions, null);
      assert.equal(fields.netSalary, 58443);
    } finally {
      globalThis.fetch = previousFetch;
      if (previousGeminiKey) process.env.GEMINI_API_KEY = previousGeminiKey;
...
function emptyGeminiPayslipResponse() {
  return new Response(JSON.stringify({
    candidates: [{
      content: {
        parts: [{
