...
- **Batch 5 — missing segments + full modeling — ✅ DONE.** All figures web-verified (post-July-2024 law).
  - **Capital gains** (special rates, both regimes): STCG 111A **20%**, LTCG 112A **12.5%** above **₹1.25L**, LTCG 112 **12.5%**; surcharge on CG/dividends **capped 15%**; 87A rebate eligibility now uses total income and never rebates LTCG.
  - **Presumptive business**: 44ADA **50%**, 44AD **8%** (self-employed step).
  - **Let-out house property**: 30% standard deduction − interest; loss set off (capped ₹2L) in the old regime only, ignored in the new.
  - **New deductions**: 80U / 80DD (₹75k/₹1.25L), 80DDB (₹40k/₹1L senior), 80EEB (₹1.5L) — old-regime only; **80CCH** (Agniveer) — both regimes.
